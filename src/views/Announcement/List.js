...
const List = () => {
  ...
  const optionFormatter = (cell, row) => {
    return (
      <ButtonGroup>
        <Link className="btn btn-success" to={"/admin/announcement/" + row.id}>
          <span>
  ...
  const handleDelete = async id => {
    try {
      const url = ApiUrl + "/announcement/delete";
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      const url = ApiUrl + "/announcement/multidelete";
  ...
  const searchData = async keywords => {
    try {
      const url = ApiUrl + "/announcement/search";
